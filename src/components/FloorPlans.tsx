  ...
  return (
    <section id="floor-plans" className="py-16 md:py-24 bg-background">
      <div className="container mx-auto px-4 lg:px-8">
        ...
        {/* Plan Selector - RESPONSIVO: Grid no mobile, Flex no desktop */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:flex lg:justify-center gap-4 mb-8 md:mb-12">
          {plans.map((plan) => (
            <button
              key={plan.id}
              onClick={() => setActivePlan(plan)}
              className={`w-full lg:w-auto px-4 py-3 md:px-6 md:py-3 rounded-lg font-medium transition-all text-sm md:text-base ${
        ...
        >
          {/* Floor Plan Image */}
          <div className="bg-card rounded-2xl p-4 md:p-8 shadow-soft border border-border order-1 lg:order-none">
            <img
          ...
          <div className="order-2 lg:order-none">
            ...
            <div className="grid grid-cols-3 gap-3 md:gap-6 mb-8">
              <div className="text-center p-3 md:p-4 bg-secondary rounded-lg">
                ...
                <div className="text-muted-foreground text-xs md:text-sm">Quartos</div>
              </div>
              <div className="text-center p-3 md:p-4 bg-secondary rounded-lg">
                <div className="font-display text-xl md:text-2xl font-bold text-foreground">
                  {activePlan.bathrooms}
                </div>
                <div className="text-muted-foreground text-xs md:text-sm">Banhos</div>
              </div>
              <div className="text-center p-3 md:p-4 bg-secondary rounded-lg">
                <div className="font-display text-xl md:text-2xl font-bold text-foreground">
                  {activePlan.parking}
                </div>
                <div className="text-muted-foreground text-xs md:text-sm">Vagas</div>
              </div>
            </div>
            ...
            <ul className="space-y-3 mb-8">
              {activePlan.features.map((feature, index) => (
                <li key={index} className="flex items-start gap-3">
                  <div className="mt-1 w-5 h-5 min-w-[1.25rem] rounded-full bg-accent/20 flex items-center justify-center">
            ...
            <Button
              ...
              size="lg"
              className="w-full sm:w-auto bg-accent text-accent-foreground hover:bg-gold-dark shadow-gold"
            >
